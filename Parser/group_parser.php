...
if(isset($_POST["action"]) && $_POST['action'] == "join_group"){
	...
	$query = mysqli_query($con, "SELECT id, invrule FROM groups WHERE name='$group' LIMIT 1");
...
if(isset($_POST["action"]) && $_POST['action'] == "approve_member"){
	...
	$query = mysqli_query($con, "SELECT id FROM gmembers WHERE gname='$g' AND mname='$u' AND approved='0' LIMIT 1");
	$numrows = mysqli_num_rows($query);
	if($numrows < 1){
    	exit();
	}
...
if(isset($_POST["action"]) && $_POST['action'] == "quit_group"){
	...
	$query = mysqli_query($con, "SELECT id FROM gmembers WHERE gname='$gS' AND mname='$uS' AND approved='1' LIMIT 1");
	$numrows = mysqli_num_rows($query);
	if($numrows < 1){
    	exit();
	}
...
if(isset($_POST["action"]) && $_POST['action'] == "add_admin"){
	...
	// Make sure already member
	$query = mysqli_query($con, "SELECT id FROM gmembers WHERE gname='$gS' AND mname='$n' AND approved='1' LIMIT 1");
	$numrows = mysqli_num_rows($query);
	if($numrows < 1){
    	exit();
	}
	...
	// Verify admin status
	$query = mysqli_query($con, "SELECT id FROM gmembers WHERE gname='$gS' AND mname='$uS' AND admin='1' LIMIT 1");
	$numrows = mysqli_num_rows($query);
	if($numrows < 1){
    	exit();
	}
