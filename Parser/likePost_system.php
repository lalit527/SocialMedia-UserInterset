...
if(isset($_POST['action']) && $_POST['action'] == "status_post"){
	...
	$sql = "SELECT COUNT(id) FROM likes WHERE name='$account_name' LIMIT 1";
	$query = mysqli_query($con, $sql);
	$row = mysqli_fetch_row($query);
	if($row[0] < 1){
	   mysqli_close($con);
	   echo "likes_no_exists";
	   exit();	
	}
	...
	$friends = array();
	$sql = "SELECT username FROM usernotification WHERE accountname='$account_name' AND type='$type'";
	$query = mysqli_query($con, $sql);
	...
	for($i=0; $i < count($friends); $i++){
		$friend = $friends[$i];
		$app = "Status";
		$note = $log_username.' posted on: <br /><a href="like.php?likes='.$account_name.'#status_'.$id.'">'.$account_name.'&#39;s Profile</a>';
