...
if(isset($_POST['action']) && $_POST['action'] == 'post'){
    ...
    $sql = "INSERT INTO status(account_name, author, type, data, postdate) VALUES ('$log_username','$log_username','d','$fl',now())";
	$query = mysqli_query($con, $sql);
...
for($x=2;$x<$arrlength;$x++) {
  //echo $files1[$x]."|";
  $file2 = get_rand_id(12).'.jpg';
 
	rename($dir.'/'.$files1[$x],$dir.'/'.$file2);
		//echo $files2;
		$sql = "INSERT INTO photos (user, gallery, filename, uploaddate) VALUES ('$log_username','$fl','$file2',now())";
		$query = mysqli_query($con,$sql);
...
if (isset($_FILES['upload_files']) && $_FILES['upload_files']['error'] != 0) {    
			...
			$_SESSION['folder'] = get_rand_id(24);
