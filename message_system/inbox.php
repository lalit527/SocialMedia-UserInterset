...
if($statusnumrows > 0){
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		$pm_replies = "";
		$query_replies = mysqli_query($con, "SELECT sender, message, senttime FROM pvtmsg WHERE parent='$pmid' ORDER BY senttime ASC");
...
function replyToPm(pmid,user,ta,btn,osender){	
	...
	_(btn).disabled = true;
	var ajax = ajaxObj("POST", "message_system.php");
