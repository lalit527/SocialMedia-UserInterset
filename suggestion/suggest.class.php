...
if($keyword != '')
$query = 'SELECT username,dplink ' .
'FROM users ' .
'WHERE username LIKE "' . $keyword . '%" OR firstname LIKE "' . $keyword . '%" OR lastname LIKE "' . $keyword . '%"';
// if the keyword is empty build a SQL query that will return no results
else
$query = 'SELECT username,dplink ' .'FROM users ' .
...
$output .= '<link> <img src="Users/'.$row['username'].'/'. $row['dplink'] .'" ></link>';
}
